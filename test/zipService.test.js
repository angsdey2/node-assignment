...
describe("Custom-Filter-Service", () => {
  test("Find-By-No-Attribute", async function() {
    await triggerLambda(noParamLoad).catch(err => {
      expect(err).toEqual("No Query parameter specified");
    });
  });

  test("Find-By-No-Attribute", async function() {
    await triggerLambda(invalidAttributeLoad).catch(err => {
      expect(err).toEqual("Invalid queryParam provided: someInvalidAttr");
    });
  });
  ...
  test("Find-Nearest-Location-By-missing-Lat", async function() {
    await triggerLambda(missingLat).catch(e => {
      expect(e).toEqual("Latitude and Longitude both must be specified");
    });
  });
});
